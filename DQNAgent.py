...
class DQNAgent:
    ...
    def replay_priority(self):
        ...
        priority = [[row[0] / sum_priority, row[1], row[2]] for row in priority]
        target_f_minibatch = []
        # print priority
        for i in range(len(priority)):
            if (i < self.minibatch_size) :
                max_index = np.argmax([row[0] for row in priority])
                # minibatch.append(self.replay_memory[max_index])
                target_f_minibatch.append([priority[max_index][1], priority[max_index][2]])
        ...
        for delta, target_f in target_f_minibatch:
            J += delta * delta
